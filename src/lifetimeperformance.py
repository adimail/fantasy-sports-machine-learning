# ==============================================================================
# Lifetime Performance Calculation
#
# Author: Aditya Godse
#
# This script cleans the player stats data, updates the players from the YAML config file,
# filters only players that exist in the squad list, and calculates the lifetime performance
# scores for batting, bowling, and fielding based on matches played between 2015 and 2024.
#
# Columns expected:
...
#
# Usage:
#   python3 -m src.lifetimeperformance
#
# ==============================================================================
...
class OverallPlayerForm:
    def __init__(self):
        """
        Initializes the OverallPlayerForm with file paths and parameters.
    ...
    def filter_players_by_squad(self, df):
        ...
        Returns:
            pd.DataFrame: The filtered DataFrame containing only valid players with updated names.
        """
        try:
    ...
    def calculate_form(self, player_df):
        """
        Calculates form scores for batting, bowling, and fielding for each player
        using data between 2015 and 2024.
        Returns:
            pd.DataFrame: Aggregated form scores per player with columns for each format.
        """
        ...
        # Sort and assign match indices for exponential decay weighting.
        fixed_data.sort_values(
            by=["Player", "End Date"], ascending=[True, False], inplace=True
        )
        fixed_data["match_index"] = fixed_data.groupby("Player").cumcount()
        fixed_data["weight"] = np.exp(
            -self.config["data"]["decay_rate"] * fixed_data["match_index"]
        )
        ...
        # ------------------
        # Batting Form Calculation
        # ------------------
        batting_metrics = {}
        for metric in ["bat runs", "bat bf", "bat sr", "bat ave", "bat 4s", "bat 6s"]:
            batting_metrics[metric] = fixed_data.groupby(
        ...
        # ------------------
        # Bowling Form Calculation
        # ------------------
        bowling_metrics = {}
        for metric in ["bowl wkts", "bowl runs", "bowl econ", "bowl overs", "bowl ave"]:
            bowling_metrics[metric] = fixed_data.groupby(
                "Player", group_keys=False
            ).apply(lambda g: compute_ewma(g, metric), include_groups=False)
        ...
        # ------------------
        # Fielding Form Calculation
        # ------------------
        fielding_metrics = {}
        for metric in ["field ct", "field st", "field ct wk"]:
            fielding_metrics[metric] = fixed_data.groupby(
                "Player", group_keys=False
            ).apply(lambda g: compute_ewma(g, metric), include_groups=False)
        ...
        # Merge the three forms into one DataFrame along with player metadata.
        form_df = (
        ...
        player_months = (
            fixed_data.groupby(["Player", "Team"])["End Date"]
    ...
    def run(self):
        ...
        Executes the full workflow:
          ...
          2. Filter players based on the squad CSV.
          3. Calculate form scores using matches from 2015 to 2024.
        """
        print(
            Fore.CYAN
            + "Starting data preprocessing for 2015 - 2024 form calculation..."
        )
        df = self.load_data()
...
def GetOverallPlayerForm():
    try:
        preprocessor = OverallPlayerForm()
        preprocessor.run()
    except Exception as e:
