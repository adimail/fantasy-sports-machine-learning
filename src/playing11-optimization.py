...
class FantasyTeamOptimizer:
    ...
    def load_and_preprocess_data(self):
        ...
        try:
            ...
            print("Data preprocessing completed. Processed features columns:",
    ...
    def optimize_team_selection(self):
        ...
        try:
            ...
            merged_players = roster.merge(self.data['features'], on='Player', how='left')
            print(f"Merged roster: {merged_players.shape[0]} players; columns: {merged_players.columns.tolist()}")

            feature_cols = self.config['feature_columns']['numeric'] + self.config['feature_columns']['categorical']
            merged_players['Predicted_Points'] = self.model.predict(merged_players[feature_cols])
    ...
    def _create_advanced_features(self):
        ...
        merged['Total_Fantasy'] = merged['Fantasy_bat'] + merged['Fantasy_bowl'] + merged['Fantasy']
        ...
        for col, default_val in zip(['Role', 'Venue_Type', 'Opposition_Strength'],
                                      ['Batsman', 'Home', 'Medium']):
            if col not in merged.columns:
    ...
    def _create_optimization_model(self, players):
        ...
        prob += pulp.lpSum([
            players.loc[players['Player'] == p, 'Predicted_Points'].values[0] * player_vars[p]
    ...
    def _add_budget_constraint(self, prob, players, variables):
        ...
        if 'Salary' in players.columns:
            prob += pulp.lpSum(
                players.loc[players['Player'] == p, 'Salary'].values[0] * variables[p]
    ...
    def _solve_optimization(self):
        ...
        selected_players = [
            p for p in self.optimization_problem.variables()
    ...
    def _select_captains(self, players):
        ...
        sorted_players = sorted(
            ...
            key=lambda x: points.get(x, 0), reverse=True
        )
