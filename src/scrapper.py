...
class Scrapper:
        ...
        for year in range(start_year, end_year + 1):
            for month in range(1, 13):
                try:
                    ...
                    print(f"{Fore.RED}Error generating dates for {year}-{month}: {e}{Style.RESET_ALL}")
        return time_spans
    ...
    def scrape_player_data(self, player_type, time_spans):
        ...
        progress_bar_format = (
            "{l_bar}{bar} | "
            f"{Fore.GREEN}Elapsed: " + "{elapsed}" + f"{Style.RESET_ALL} | " +
            "{postfix}"
        )
    ...
    def clean_data(self, df, data_type):
        ...
        try:
            ...
            print(f"{Fore.RED}Error cleaning data for {data_type}: {e}{Style.RESET_ALL}")
...
def scrapeData(start_year, end_year):
    ...
    try:
        ...
        print(f"Total data downloaded overall: {Fore.YELLOW}{overall_total}{Style.RESET_ALL}")
